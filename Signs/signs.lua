...
function Signs:draw()
  ...
  if self.stat == 'show' then
    ...
    if tx < 0 then tx = self.x+10 end
    local bx , by = self.x-(tw/10) , self.y-(th/10)-115
    if bx < 0 then bx = self.x end
    love.graphics.draw(self.textBox , bx , by , 0 , 1/4 , 1/4)
    love.graphics.printf(string.sub(self:queryText() , 0 , self.textPosition),self.font,tx , ty , self.textBox:getWidth()/2.6,'left',0 , 0.6 ,0.6)
  end
end
