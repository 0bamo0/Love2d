...
function Pigs.new(x, y, width, height, speed , health)
    ...
    instance.animation.dead = anim8.newAnimation(instance.grid("20-23", 1), 0.2)
...
function Pigs:checkNoGround(dt)
    local query =
        world:queryRectangleArea(self.x + self.direction * 5, self.y + self.height / 2, 2, 2, {"Ground", "Platforms"})
    if #query == 0 then
        return true
    end
end
...
function Pigs:checkWalls(dt)
    if self.collider:enter("Walls") then
        return true
    end
end
